...
**客户端接入 → 搜索访问凭证** 显示每个 `sa_…` 凭证的累计与本月（UTC）统计，点击页面 **刷新** 更新。统计按不可变的凭证 ID 归属，同名凭证分别计数；撤销后仍可查看既有统计。接口 `GET /api/tokens` 在每条凭证中附带 `usage.lifetime`、`usage.month` 和 `usage.month_start`，需要管理员登录。
