...
import { Activity, Archive, ArrowRight, Check, ChevronRight, CircleHelp, FlaskConical, KeyRound, LayoutDashboard, ListFilter, LogOut, Menu, Plug, Plus, RefreshCw, ShieldCheck, SlidersHorizontal, X } from 'lucide-react';
...
type Page = 'overview' | 'keys' | 'profiles' | 'logs' | 'playground' | 'connections' | 'backups';
const pages = [
  ...
  { id: 'backups', label: '备份与迁移', icon: Archive },
] as const;
...
export default function App() {
  ...
  switch (page) {
    ...
    case 'backups': content = <Backups reload={load}/>; break;
  }
  return <div className="app-shell">{menu && <button className="sidebar-overlay" aria-label="关闭导航" onClick={() => setMenu(false)}/>}
    <aside className={`sidebar ${menu ? 'open' : ''}`}><Brand/><div className="workspace-label"><span className="workspace-avatar">S</span><span>我的搜索工作台<small>Personal workspace</small></span><ChevronRight size={14}/></div>
      ...
      <div className="sidebar-bottom"><div className="local-note"><span className="status-dot"/><span>本地网关已连接<small>HTTP + MCP · v0.3.0</small></span></div><button className="nav-item" onClick={() => navigate('connections')}><CircleHelp size={18}/>接入与使用说明</button><button className="nav-item" onClick={async () => { await api('/session', { method: 'DELETE' }); setAuth(false); setData(null); }}><LogOut size={18}/>退出登录</button></div>
    </aside>
