...
export function channelQuotas(keys: KeyPublic[]) {
  return PROVIDERS.flatMap(provider => {
    ...
    const known = channel.filter(key => key.usage?.status === 'ok' && (key.usage.account || key.usage.balance || key.usage.request_quota || key.usage.money_balance || key.usage.organization_balance));
    const scopes = new Map<string, KeyPublic>();
    for (const key of [...known].sort((a, b) => a.usage!.synced_at.localeCompare(b.usage!.synced_at))) scopes.set(key.usage!.balance?.scope || key.usage!.request_quota?.scope || key.usage!.money_balance?.scope || key.usage!.organization_balance?.scope || key.id, key);
    const unique = [...scopes.values()];
    const credits = unique.filter(key => !key.usage!.money_balance && !key.usage!.organization_balance);
    ...
    return [{
      ...
      failedCount: channel.filter(key => key.usage_error).length,
